...
% lambda=0.001;%LCA threshhold face or finger separate

lambda=0.001 % whiten together
...
%datadouble =0; (faces) or datadouble=2; (fingerprints)
...
%        Lp=[Lp1;Lp2];
% 
% r2=randperm(size(Lp,1)); 
% 
% 
% 
% Lp=Lp(r2,:);
% %                  
%     %  whitens entire library faces and fingerprints together  
%       %  L = bsxfun(@minus,L,mean(L)); %remove mean
%      fX = fft(fft(Lp,[],2),[],3);      %fourier transform of the images
%      spectr = sqrt(mean(abs(fX).^2));  %Mean spectrum
%      Lp = ifft(ifft(bsxfun(@times,fX,1./spectr),[],2),[],3);   %whitened Lp
%     
%      L=Lp; 
%      Lp=[Lp1;Lp2];
% % 
...
for k=[1:size(L,2)]% 1:size(L,2) 1:size(L,2)] %LCA loop, tests each picture one at a time (kth picture); (size(L,2))= N*M*2=560
    ...
    % Lp(:,k)=0; % delete image vector k from library
...
for i =1:size(patient_names,2) % loop through patients
    ...
    D=[]; %clear the Dictionary for each new patient
...
%face or fingers, 0,2
t=.01; %time step      i = 100 lambda = 
h=.000001; %step size
% 
% % face and fingers whiten together, 1
%  t=.08; %time step      i = 100
%  h=.000001; %step size
...
for i=1:100
    ...
    % converges to the sparsest represtation of y with L*a
